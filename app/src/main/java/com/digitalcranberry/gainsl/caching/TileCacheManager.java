...
/**
 * Yummily borrowed from the OSM BOnus pack. Everything below is from them, but is LGPL, so ok to close-source
 * ------------
...
public class TileCacheManager {
    ...
    public TileCacheManager(final MapView mapView){
    ...
    public boolean loadTile(OnlineTileSourceBase tileSource, MapTile tile){
        ...
        try {
            ...
            if (line.getStatusCode() != 200) {
                Log.w(DEBUGTAG, "Problem downloading MapTile: " + tile + " HTTP response: " + line);
                return false;
            }

            final HttpEntity entity = response.getEntity();
            if (entity == null) {
                Log.w(DEBUGTAG, "No content downloading MapTile: " + tile);
                return false;
            }
            ...
            Log.w(DEBUGTAG, "UnknownHostException downloading MapTile: " + tile + " : " + e);
        } catch (final FileNotFoundException e) {
            Log.w(DEBUGTAG, "Tile not found: " + tile + " : " + e);
        } catch (final IOException e) {
            Log.w(DEBUGTAG, "IOException downloading MapTile: " + tile + " : " + e);
        } catch (final Throwable e) {
            Log.e(DEBUGTAG, "Error downloading MapTile: " + tile, e);
        } finally {
    ...
    protected class DownloadingTask extends CacheManagerTask {
        ...
        protected int downloadArea(){
            ...
            if (mTileProvider.getTileSource() instanceof OnlineTileSourceBase)
                ...
                Log.e(DEBUGTAG, "TileSource is not an online tile source");
                return 0;
            }
